...
/**
 * 九联区块奇数中心维度的传送器，复用前置 AbstractDimensionTeleporter 的完整传送链路：
 * 找落点、POI 检索、传送、配置界面（DimensionConfigMenu）触发逻辑全部继承自前置。
 */
public class NineChunkOddDimTeleporter extends AbstractDimensionTeleporter {
    @Override
    protected ResourceKey<Level> getDimensionKey() {
        return NineChunkOddDimensions.NINE_CHUNK_ODD_KEY;
    }

    @Override
    protected Supplier<Block> getTeleportBlock() {
        return ModBlocks.NINE_CHUNK_ODD_DIMENSION_BLOCK;
    }

    @Override
    protected Holder<PoiType> getPOI() {
        return ModPOIs.NINE_CHUNK_ODD_TELEPORT_POI;
    }
}
